...
void SwapPrio(INT8U p1, INT8U p2);
        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
					if (OSPrioCur!=63 && OSTCBCur->CompTime <= 0) {
						OS_EXIT_CRITICAL();
						return;
					}
					INT8U From = OSPrioCur, To = OSPrioHighRdy;
					INT16U FromID = OSTCBCur->OSTCBId, ToID = OSTCBPrioTbl[OSPrioHighRdy]->OSTCBId;
					...
					printf("\n%5d Preempt %10d(%d)%10d(%d)", OSTimeGet(), FromID, From, ToID, To);
                }
            }
        }
        OS_EXIT_CRITICAL();
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
			INT8U From = OSPrioCur;
			INT16U FromID = OSTCBCur->OSTCBId;
			boolean PrioChange = 0;
			for (int i = 0; i < 2; i++) {
				for (int j = 0; j < 2 - i; j++) {
					OS_TCB *t1 = OSTCBPrioTbl[j + 1];
					OS_TCB *t2 = OSTCBPrioTbl[j + 2];
					if (t2->Deadline < t1->Deadline) {
						PrioChange = 1;
						SwapPrio(t1->OSTCBPrio, t2->OSTCBPrio);
					}
				}
			}
            ...
            OS_SchedNew();
            if (OSPrioHighRdy != OSPrioCur || PrioChange) {          /* No Ctx Sw if current task is highest rdy     */
				INT8U To = OSPrioHighRdy;
				INT16U ToID = OSTCBPrioTbl[OSPrioHighRdy]->OSTCBId;
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
#if OS_TASK_PROFILE_EN > 0
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
				...
				printf("\n%5d Complete%10d(%d)%10d(%d)", OSTimeGet(), FromID, From, ToID, To);
            }
        }
    }
    OS_EXIT_CRITICAL();
...
void SwapPrio(INT8U p1, INT8U p2) {
	OSTaskChangePrio(p1, 60);
	OSTaskChangePrio(p2, p1);
	OSTaskChangePrio(60, p2);
}
